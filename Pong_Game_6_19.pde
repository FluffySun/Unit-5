...
void showIntro() {
  ...
  // 1 Player Button
  boolean hover1 = dist(mouseX, mouseY, width/2, 260) < 60;
  fill(hover1 ? color(255, 200, 255) : color(150, 100, 200));
  ellipse(width/2, 260, 100, 100);

  // 2 Player Button
  boolean hover2 = dist(mouseX, mouseY, width/2, 410) < 60;
  fill(hover2 ? color(255, 100, 100) : color(200, 80, 80));
...
void resetBall() { //I DID NOT LEARN TO DO RANDOM LIKE THIS
  ballX = width/2;
  ballY = height/2;
if (random(1) < 0.5) vx = random(1.5,3);
else vx = 5;
  vy = random(-1.5, 3);
  if (vy==0) vy=1;
  waiting = true;
